...
export default function HeadToHead({ profile, predictions, allProfiles, allPredictions, finishedMatches }: HeadToHeadProps) {
    ...
    const stats = useMemo(() => {
        ...
        finishedMatches.forEach(match => {
            ...
            if (userPred) {
                userPoints += userPred.points || 0;
                userPointsArray.push(userPred.points || 0);
                if (userPred.points === 5) userPerfect++;
                if ((userPred.points || 0) === 0) userZero++;
            }
            if (oppPred) {
                oppPoints += oppPred.points || 0;
                oppPointsArray.push(oppPred.points || 0);
                if (oppPred.points === 5) oppPerfect++;
                if ((oppPred.points || 0) === 0) oppZero++;
            }
            ...
            if (userPred && oppPred) {
                ...
                if ((userPred.points || 0) > 0 && (oppPred.points || 0) === 0) {
                    userGainedOppZero++;
                } else if ((oppPred.points || 0) > 0 && (userPred.points || 0) === 0) {
                    oppGainedUserZero++;
                }
            }
        });
